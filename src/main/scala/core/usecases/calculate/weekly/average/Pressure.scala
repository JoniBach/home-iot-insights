...
/** Use case for calculating weekly average pressure. This is designed to be run
  * weekly (e.g., at 1 AM) to process the previous week's data.
  */
final class CalculateWeeklyAveragePressure[F[_]: Monad](
  ...
  // Weekly Average Pressure insight type ID
  private val insightTypeId = UUID.fromString(
    "c160c68c-0b82-4e1a-8bd8-6aab738c0266"
  ) // Matches PressureAverage in InsightType

  /** Executes the weekly average pressure calculation. Processes data from
    * midnight to midnight of the previous week.
    *
    * @return
...
object CalculateWeeklyAveragePressure {
  def default[F[_]: Monad](
      ...
      insightsPort: InsightsPort[F]
  ): CalculateWeeklyAveragePressure[F] = {
    ...
    new CalculateWeeklyAveragePressure[F](
