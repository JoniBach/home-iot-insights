...
/** Use case for calculating monthly average pressure. This is designed to be run
  * monthly (e.g., at 1 AM) to process the previous month's data.
  */
final class CalculateMonthlyAveragePressure[F[_]: Monad](
  ...
  // Monthly Average Pressure insight type ID
  private val insightTypeId = UUID.fromString(
    "c160c68c-0b82-4e1a-8bd8-6aab738c0266"
  ) // Matches PressureAverage in InsightType

  /** Executes the monthly average pressure calculation. Processes data from
    * midnight to midnight of the previous month.
    *
    * @return
...
object CalculateMonthlyAveragePressure {
  def default[F[_]: Monad](
      ...
      insightsPort: InsightsPort[F]
  ): CalculateMonthlyAveragePressure[F] = {
    ...
    new CalculateMonthlyAveragePressure[F](
