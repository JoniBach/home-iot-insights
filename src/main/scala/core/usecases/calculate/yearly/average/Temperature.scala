...
/**
 * Use case for calculating yearly average temperatures.
 * This is designed to be run yearly (e.g., at 1 AM) to process the previous year's data.
 */
final class CalculateYearlyAverageTemperature[F[_]: Monad](
  ...
  // Yearly Average Temperature insight type ID
  private val insightTypeId = UUID.fromString("56da8d03-a64e-4395-8a7c-ebedbf8b122b")

  /**
   * Executes the yearly average temperature calculation.
...
object CalculateYearlyAverageTemperature {
  /**
  ...
  def default[F[_]: Monad](
  ...
  ): CalculateYearlyAverageTemperature[F] = {
    ...
    new CalculateYearlyAverageTemperature[F](
