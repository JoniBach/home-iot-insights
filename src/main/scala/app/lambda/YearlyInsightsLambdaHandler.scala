...
import core.usecases.calculate.yearly.average.{CalculateYearlyAverageTemperature, CalculateYearlyAverageHumidity, CalculateYearlyAveragePressure}
import io.circe.generic.auto._
import io.circe.syntax._
import io.circe.Printer
import core.entities.Insight

class YearlyInsightsLambdaHandler extends RequestHandler[Unit, String] {
  ...
  override def handleRequest(input: Unit, context: Context): String = {
    ...
    try {
      ...
      val averageTemperature = CalculateYearlyAverageTemperature.default[IO](
        readingsPort,
      ...
      val averageHumidity = CalculateYearlyAverageHumidity.default[IO](
        readingsPort,
        deviceRoomBuildingsPort,
        sensorsPort,
        insightsPort
      ...
      val averagePressure = CalculateYearlyAveragePressure.default[IO](
        readingsPort,
        deviceRoomBuildingsPort,
        sensorsPort,
        insightsPort
      ...
      logger.log("=== Starting Yearly Average Temperature Calculation ===")
      val temperatureInsights = averageTemperature.execute().unsafeRunSync()
      
      logger.log("=== Starting Yearly Average Humidity Calculation ===")
      val humidityInsights = averageHumidity.execute().unsafeRunSync()
      
      logger.log("=== Starting Yearly Average Pressure Calculation ===")
